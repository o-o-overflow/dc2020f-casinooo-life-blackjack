from .variables import VariableStore, Variable, ScratchVariable, CustomVariable, PointerVariable, ReferenceVariable, _id_gen
...
class FileInterpreter:
    def __init__(self, tree: GrammarTree):
        ...
        self.lists = {"sub": self.subs,
                      ...
                      "newline": []}
        for stmt in stmts:
            blkname = stmt["_block_name"]
            tmp = self.file_types[blkname](self.global_store, self.inlines, stmt)
            self.lists[stmt["_block_name"]].append(tmp)
...
class StmtInterpreter(GlobalLocalStoreHelper):
    def __init__(self, global_store: VariableStore, local_store: VariableStore, inlines, tree: GrammarTree):
        ...
        self.stmt_types = {
            ...
            "send": SendInterpreter,
            "read": ReadInterpreter,
            "rand": RandInterpreter,
...
class SendInterpreter(GlobalLocalStoreHelper):

    def __init__(self, global_store: VariableStore, local_store: VariableStore, inlines, tree: GrammarTree):
        super().__init__(global_store, local_store, inlines)
        self.value = self.parse_generic_value(tree["generic_value"])
        self.toip = self.parse_generic_value(tree['generic_value_2'])

    def __repr__(self):
        return str(self.var) + " = " + str(self.value)

    def compile(self):
        rtn, scratch = self.collect_value(self.toip)
        rtn_val, scratch_val = self.collect_value(self.value)
        rtn.extend(rtn_val)

        # if scratch_val is not self.value:
        rtn.append(("send", scratch, scratch_val))
        self.free_scratch(scratch_val)
        self.free_scratch(scratch)
        return rtn

class RandInterpreter(GlobalLocalStoreHelper):
    ...
    def __repr__(self):
        return "rand " + str(self.var)

    def compile(self):
        rtn, scratch = self.collect_value(self.var)

        # if scratch_val is not self.value:
        rtn.append(("rand", scratch))
        self.free_scratch(scratch)
        return rtn

class ReadInterpreter(GlobalLocalStoreHelper):

    def __init__(self, global_store: VariableStore, local_store: VariableStore, inlines, tree: GrammarTree):
        super().__init__(global_store, local_store, inlines)
        self.network_data = self.parse_generic_value(tree["generic_value"])
        self.var = self.parse_generic_var(tree["generic_var"])

    def __repr__(self):
        return "read " + str(self.network_data) + " " + str(self.var)

    def compile(self):
        rtn, scratch = self.collect_value(self.network_data)
        rtn_val, scratch_val = self.collect_value(self.var)
        rtn.extend(rtn_val)

        rtn.append(("read", scratch, scratch_val))
        self.free_scratch(scratch_val)
        self.free_scratch(scratch)
        return rtn
    ...
    def __init__(self, tree: GrammarTree):
        ...
        if tree["_block_name"] == "number":
            try:
                ...
                if tree["value"].startswith("0b"):
                    self.val = int(tree["value"],2)
                else:
                    self.val = int(tree["value"], 16)
        else:
