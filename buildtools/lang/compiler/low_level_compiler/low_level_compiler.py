...
class FileInterpreter:
    opcodes = ["__MNZ__", "__MLZ__", "__ADD__", "__SUB__", "__AND__",
               "__OR__", "__XOR__", "__ANT__", "__SL__", "__SRL__", "__SRA__", "__SEN__", "__REC__"]
    ...
    def __init__(self, instruction_list, global_store: VariableStore):
        ...
        self.compilers = {
            ...
            "send": self.send_interpreter,
            "read": self.read_interpreter,
            "rand": self.rand_interpreter
        }
    ...
    def rand_interpreter(self, variable):
        """
        ...
        return ["RND 0 0 {}".format(self.parse_result(variable))]

    def send_interpreter(self, value, toip):
        """
        Sends value to ip
        ...
        return ["SEN {} {} -1".format(self.parse_variable(value), self.parse_variable(toip))]

    def read_interpreter(self, network_data, target_address):
        """
        Reads from stream into address

        :param variable:
        :param value:
        :return:
        """
        return ["REC 1 {} {}".format(self.parse_variable(network_data),
                                      self.parse_result(target_address))]
    ...
    def parse_variable(self, variable: Variable) -> str:
        ...
        if variable.is_pointer:
            if variable.points_to.is_array:
                # TODO: not techinically a pointer
    ...
    def parse_result(self, variable: Variable) -> str:
        ...
        if variable.is_pointer:
            if variable.points_to.is_array:
                # TODO: not techinically a pointer
                return ""+str(variable.points_to.offset + variable.plus)
