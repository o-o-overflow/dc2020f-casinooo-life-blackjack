...
uint16_t getValue(int mempos){
    ...
    for (int ypos=15; ypos >= 0; ypos--){
        ...
        if (imp->getcell(MEMSTART_X+mempos*22, MEMSTART_Y+ypos*22) == 7){
            bitstr += '1';
        } else {
            bitstr += '0';
        }
...
bool init_game_data(){
    if (shm_id == -1){
        shm_id = shmget(449988 , MAP_SIZE, IPC_CREAT | 0666);
        ...
        shared_data = (uint8_t *) shmat(shm_id, NULL, 0);

        memset(shared_data, 0, 512);
...
case 's':
   ...
   */
   ...
   //fork here
    ...
    uint16_t cards[16] = {0xA313,0x198A,0xAA14,0x9912,0xAA14,0x1A9B,0x830B,0x930C,0x660C,0x8911,0x460A,0x918A,0x390C,0x920B,0x198A,0xA30D};
    uint16_t bank[16] = {100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100};
    uint16_t bets[16] = {69,5,10,99,14,15,20,30,40,50,60,70,80,90,55,32};
    for (int i =0; i < 16; i++){
        setMemory(cards[i], 33+i);
        setMemory(bank[i], 49+i);
        setMemory(bets[i], 65+i);
    }
   for (;;) {
      ...
      uint16_t actionVal = getValue(11);
